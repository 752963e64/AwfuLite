...
int ren_draw_text(RenFont *font, const char *text, int x, int y, RenColor color) {
  ...
  while (*p) {
    ...
    ren_draw(set->image, &rect, x + g->xoff, y + g->yoff, color);
    x += g->xadvance;
  }
  return x;
}
...
void ren_draw_image(const char *fullpath) {
    ...
    w = 800;
    h = 600;
    SDL_Window* win = SDL_CreateWindow(fullpath, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, w, h, SDL_WINDOW_OPENGL);
    SDL_Renderer* renderer = SDL_CreateRenderer(win, -1, SDL_RENDERER_ACCELERATED);

    /* Open the image file */
    texture = IMG_LoadTexture(renderer, fullpath);
    if (!texture) {
    ...
    /* Show the window */
    SDL_SetWindowTitle(win, fullpath);
